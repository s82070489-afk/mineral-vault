...
import { isLocalYesterday, isSameLocalDay } from './date'
...
export function checkIn(state: GameState, now: number): GameState {
  ...
  const history = isConsecutive ? [...attendance.history, true] : [true]
  ...
  return {
    ...state,
    attendance: { streak: nextStreak, lastCheckInAt: now, history },
    minerals: {
      ...state.minerals,
